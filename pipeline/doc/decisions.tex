...
\usepackage{booktabs,array,amsmath,amssymb,enumitem,xcolor}
\usepackage[T1]{fontenc}
\usepackage{sectsty}\allsectionsfont{\sffamily}
\usepackage{microtype}
\setlength{\parskip}{5pt}\setlength{\parindent}{0pt}
\newcommand{\pp}{\,pp}
\title{\sffamily Firm-level price wedges for pricewedge.com\\[4pt]\large Three design decisions and the evidence behind them}
...
The evidence is in two companion documents: \texttt{quality.pdf} (the mapping
and the level tests) and \texttt{signals.pdf} (every characteristic's
...
(\texttt{quality.pdf}). That difference is not the decision here.
...
\begin{center}\small
\begin{tabular}{p{3.6cm}p{5.2cm}rrrr}\toprule
...
1. Published construction (value-weighted wedges, equal-weighted characteristics) & ``the number the paper's method assigns'' & 0.64 & $-18$ & 0.95 & $+9$\\[3pt]
2. Per dollar (value-weighted wedges and characteristics) & ``a dollar in firms like this one is mispriced by $X$'' & 0.67 & $-3$ & 0.79 & $+1$\\[3pt]
3. Per firm, aggregate pinned (both portfolio sets, each with matching characteristics) & ``firms like this one are mispriced by $X$ on average; dollar-weighted, mispricing is zero'' & 0.86 & $-1$ & 0.76 & $0$\\
\bottomrule
\end{tabular}
\end{center}

\emph{Intercepts in pp; ``VW mean'' is the value-weighted average of the
candidate's firm-level wedges, which should be near zero.}
...
\texttt{doc/quality.pdf}, \texttt{doc/signals.pdf} & the evidence\\
\texttt{Website/Literature/} & the source papers, indexed in \texttt{\_index.md}\\
\bottomrule
\end{tabular}
